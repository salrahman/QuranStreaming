...
const STORAGE_KEY = 'quran-audio-v6.3';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  reciters.forEach((r, idx) => {
    const opt = document.createElement('option'); opt.value = idx; opt.textContent = r.name; reciterSelect.appendChild(opt);
  });

  // populate surah list with separate english and arabic spans
  function renderList() {
    surahList.innerHTML = '';
    surahs.forEach((s, i) => {
      const li = document.createElement('li');
      li.setAttribute('role','button'); li.tabIndex = 0; li.dataset.index = i; li.setAttribute('aria-pressed', i === current ? 'true' : 'false');
  ...
  audio.addEventListener('loadedmetadata', ()=>{
    if(pendingResume!==null){ try { audio.currentTime = Math.min(pendingResume, Math.max(0, audio.duration-1)); announce('Resumed at ' + Math.floor(pendingResume) + 's'); } catch(e){} pendingResume=null; }
  });
  ...
  audio.addEventListener('error', (e)=>{
    ...
    if(currentReciter !== 0 && !fallbackTried){
      fallbackTried = true; reciterSelect.value = '0'; announce('Switching to ' + reciters[0].name); const lastTime = audio.currentTime || 0; loadSurah(current, true, lastTime);
    }
  });

  reciterSelect.addEventListener('change', ()=>{
    const idx = parseInt(reciterSelect.value||'0',10); announce('Reciter changed to ' + reciters[idx].name); const lastTime = audio.currentTime || 0; try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ lastSurah: current, lastReciter: idx, lastTime: Math.floor(lastTime) })); } catch(e){}
  ...
  document.addEventListener('keydown', (e)=>{
    const tag = document.activeElement && document.activeElement.tagName; if(['INPUT','SELECT','TEXTAREA'].includes(tag)) return;
